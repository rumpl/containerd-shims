...
RUN <<EOT
    set -ex
    os=$(xx-info os)
    curl -sSf https://raw.githubusercontent.com/WasmEdge/WasmEdge/master/utils/install.sh | bash -s -- --version ${WASMEDGE_VERSION} --platform ${os^} --machine $(xx-info march)
EOT
...
# Runwasi shims
COPY --link --from=build-runwasi /root/.wasmedge/lib/libwasmedge.so.0.0.2 /libwasmedge.so.0.0.2
